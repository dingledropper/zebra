...
impl Cookie {
    ...
    ///
    /// Uses constant-time comparison to avoid leaking the secret via
    /// timing side channels. The length short-circuit is intentional —
    /// cookie length is fixed at 44 bytes (base64 of 32 random bytes)
    /// and is not secret.
